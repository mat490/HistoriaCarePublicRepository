...
public class JWTTokenProvider {
    ...
    public String generateJwtToken(Authentication authentication, Usuario user) {
        Claims claims = Jwts.claims().setSubject("UNAM").setIssuer(user.getCorreoElectronico())
                .setAudience("JAVA");
        ...
        claims.put("issname", user.getCorreoElectronico());
        key = Keys.hmacShaKeyFor(secret.getBytes());
        log.info("JWT Expiration time (ms): {}", jwtExpirationInMs);
        return Jwts.builder()
                .setClaims(claims)
    ...
    public boolean validateJwtToken(String authToken) {
        try {
            ...
            log.info("JWT Expiration time (ms): {}", this.jwtExpirationInMs);
